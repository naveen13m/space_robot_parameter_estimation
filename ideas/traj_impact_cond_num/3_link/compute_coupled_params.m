% Given the parameters ofindividual links, it computes the value of coupled
...
function pi = compute_coupled_params(x)
    ...
    pi(1) = Izz0 + norm([0.5 + a0x, a0y])^2 * (m1 + m2);
    pi(2) = m0 + m1 + m2;
    pi(3) = m0 * a0x + (0.5 + a0x) * (m1 + m2);
    pi(4) = m0 * a0y + a0y * (m1 + m2);
    pi(5) = Izz1 + m2;
    pi(6) = m1 * a1x + m2;
    pi(7) = m1 * a1y;
    pi(8) = Izz2;
