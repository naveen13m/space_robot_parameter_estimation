...
for i=2:length(T)
    if T(i)>s
        ...
        BX(j,:)=[sbf(1,1:5) sbf(1,1)];
        BY(j,:)=[sbf(2,1:5) sbf(2,1)];
        BZ(j,:)=[sbf(3,1:5) sbf(2,1)];
        L1X(j,:)=[sof(1,2:3) stf(1,3)];
        L1Y(j,:)=[sof(2,2:3) stf(2,3)];
        L1Z(j,:)=[sof(3,2:3) stf(3,3)];
        L2X(j,:)=[sof(1,4) stf(1,4)];
        L2Y(j,:)=[sof(2,4) stf(2,4)];
        L2Z(j,:)=[sof(3,4) stf(3,4)];
        j=j+1;
        s=s+tskip;
    else
        continue
...
for i=1:length(time)
    ...
    hold on;
%     fill3(BX(i,1:3),BY(i,1:3),BZ(i,1:3),[4 4 4 4],BX(i,4:5),BY(i,4:5),BZ(i,4:5),[4 4 4 4]...
