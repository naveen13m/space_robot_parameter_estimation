function [reg_mat] = regressor_matrix(robot_make,...
    ...
    num_arms = 0;
    for curr_initial_link_index = arm_initial_link_index
        num_arms = num_arms + 1;
        base_arm_joint_position_base_frame = ...
            [link_length_DH(curr_initial_link_index);
            -joint_offset(curr_initial_link_index) * ...
            sin(joint_twist_angle(curr_initial_link_index)); ...
             joint_offset(curr_initial_link_index) * ...
             cos(joint_twist_angle(curr_initial_link_index))];
        base_arm_joint_position_sensor_frame_sensor_frame(:, num_arms) = ...
            base_arm_joint_position_base_frame ...
    ...
    base_sensor_ang_velocity = statevar(:, 10 + num_links_without_base :...
        12 + num_links_without_base); % ZXY order
    temp = base_sensor_ang_velocity(:, 1);
    base_sensor_ang_velocity(:, 1) = base_sensor_ang_velocity(:, 2);
    base_sensor_ang_velocity(:, 2) = base_sensor_ang_velocity(:, 3);
    base_sensor_ang_velocity(:, 3) = temp;
    base_sensor_ang_velocity = base_sensor_ang_velocity.';
...
%         joint_ang_speed];
    ...
    % Compute rotation matrix of link frames
    rot_mat_link_prev_link_frame = ...
        zeros(3, 3, num_instants, num_links);
    for curr_instant = 1 : num_instants
        rot_mat_link_prev_link_frame(:, :, curr_instant, 1) = ...
            rotz(rad2deg(base_sensor_orientation(1, curr_instant))) * ...
            rotx(rad2deg(base_sensor_orientation(2, curr_instant))) * ...
            roty(rad2deg(base_sensor_orientation(3, curr_instant))); % ZXY euler angle
        for curr_link_index = 2 : num_links
            curr_joint_index = curr_link_index - 1;
            joint_angle = joint_ang_position(curr_joint_index, curr_instant);
            rot_mat_link_prev_link_frame(:, :, curr_instant, curr_link_index) = ...
                DH_to_rot_mat(joint_twist_angle(curr_link_index), joint_angle);
        end
    end
        ...
        for curr_arm_index = 1 : num_arms
            for curr_link_index = arm_initial_link_index(curr_arm_index) : ...
                ...
                if curr_link_index ==  arm_initial_link_index(curr_arm_index)
                    rot_mat_prev_link = rot_mat_link(:, :, curr_instant, 1);
                else
                    rot_mat_prev_link = rot_mat_link(:, :, curr_instant,...
                        curr_link_index - 1);
                end
    ...
    base_arm_joint_base_sensor = zeros(3, num_instants, num_arms);
    
    for curr_instant = 1 : num_instants
        for curr_arm_index = 1 : num_arms
            base_arm_joint_base_sensor(:, curr_instant, curr_arm_index) ...
                = rot_mat_link(:, :, curr_instant, 1) * ...
                base_arm_joint_position_sensor_frame_sensor_frame(:, curr_arm_index);
            for curr_link_index = arm_initial_link_index(curr_arm_index) : ...
                    arm_terminal_link_index(curr_arm_index)
                if curr_link_index ==  arm_initial_link_index(curr_arm_index)
                    curr_link_frame_position_prev_link_frame = ...
                        base_arm_joint_base_sensor(:, curr_instant, curr_arm_index);
                    prev_link_frame_position = link_frame_position(:, curr_instant, 1);
                else
                    curr_link_frame_position_prev_link_frame_prev_link_frame = ...
                ...
                link_frame_position(:, curr_instant, curr_link_index) = ...
                    prev_link_frame_position + curr_link_frame_position_prev_link_frame;
            end
        end
    end
    ...
    for curr_instant = 1 : num_instants
        for curr_arm_index = 1 : num_arms
            for curr_link_index = arm_initial_link_index(curr_arm_index) : ...
                    arm_terminal_link_index(curr_arm_index)
                link_frame_position_base_sensor_frame = ...
                    (link_frame_position(:, curr_instant, curr_link_index) - ...
                ...
                link_frame_lin_velocity(:, curr_instant, curr_link_index) = ...
                    ...
                    vec_to_mat(base_sensor_ang_velocity(:, curr_instant)) * ...
                    link_frame_position_base_sensor_frame;
                ...
                for iter_link_index = arm_initial_link_index(curr_arm_index) : ...
                        curr_link_index % check - Imp
                    iter_joint_index = iter_link_index - 1;
                    joint_angular_velocity = ...
                    rot_mat_link(:, :, curr_link_index) * ...
                    rot_mat_link_prev_link_frame(:, :, curr_link_index).' * ...
                    [0; 0; joint_ang_speed(iter_joint_index, curr_instant)];
                    link_frame_position_iter_link_frame = ...
                        (link_frame_position(:, curr_instant, curr_link_index) - ...
                        link_frame_position(:, curr_instant, iter_link_index));
                    link_frame_lin_velocity(:, curr_instant, curr_link_index) = ...
                        link_frame_lin_velocity(:, curr_instant, curr_link_index) + ...
                        vec_to_mat(joint_angular_velocity) * link_frame_position_iter_link_frame;
                    link_frame_ang_velocity(:, curr_instant, curr_link_index) = ...
                        link_frame_ang_velocity(:, curr_instant, curr_link_index) + ...
                        joint_angular_velocity;
                end
    ...
    for curr_joint_index = 1 : num_links
        figure(1);
        joint_pos_x_coord = squeeze(link_frame_position(1, :, curr_joint_index));
        joint_pos_y_coord  = squeeze(link_frame_position(2, :, curr_joint_index));
        joint_lin_vel_x_comp = squeeze(link_frame_lin_velocity(1, :, curr_joint_index));
        joint_lin_vel_y_comp = squeeze(link_frame_lin_velocity(2, :, curr_joint_index));
