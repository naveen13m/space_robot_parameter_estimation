...
%         coeff_increment = [tr_par_seed(lnt / 2 + 1 : end), 15 * pi / 2];
        start_position = tr_par_seed(1 : lnt / 2);
        coeff_increment = tr_par_seed(lnt / 2 + 1 : end);
    else
        start_position = [tr_par_seed(1 : lnt / 2), 0, 0, pi/2, 0];
        coeff_increment = [tr_par_seed(lnt / 2 + 1 : end), 15 * pi / 2, 2 * pi, 0, 13 * pi/2];
    end
