...
for curr_col = 1 : num_cols - 1
    ...
    state_error = statevar_1(:, curr_col) - statevar_2(:, curr_col);
    plot(1:1:num_rows, statevar_1(:, curr_col), ...
        1:1:num_rows, statevar_2(:, curr_col), 1:1:num_rows, state_error);
    grid on;
    title(text{curr_col});
end
hL = legend('Set-2a', 'Set-2b','error');
...
for curr_col = 1 : num_cols
    ...
    tor_error = tor_1(:, curr_col) - tor_2(:, curr_col);
    plot(1:1:num_rows, tor_1(:, curr_col), ...
        1:1:num_rows, tor_2(:, curr_col), 1:1:num_rows, tor_error);
    grid on;
    title(text{curr_col});
end
hL = legend('Set-2a', 'Set-2b','error');
